import type { FlightOffer, RankedOffer } from "../types.js";

export function dedupeCheapest(offers: FlightOffer[]): FlightOffer[] {
  const map = new Map<string, FlightOffer>();
  for (const offer of offers) {
    const existing = map.get(offer.itineraryKey);
    if (!existing || offer.price < existing.price) map.set(offer.itineraryKey, offer);
  }
  return [...map.values()];
}
...
export function detectBestDelta(currentBest: number, previousBest?: number): { drop: boolean; delta: number } {
  if (previousBest == null) return { drop: false, delta: 0 };
