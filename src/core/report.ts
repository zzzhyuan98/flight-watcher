...
import type { RankedOffer } from "../types.js";

export function renderSummary(offers: RankedOffer[], delta: number): string {
  if (!offers.length) return "No offers found from enabled adapters.";
  const best = offers[0];
...
export function writeArtifacts(basePath: string, offers: RankedOffer[], summary: string): void {
  ...
  const lines = [
    ...
    ...offers.map(
      (o, i) =>
        `${i + 1}. **${o.currency} ${o.price.toFixed(2)}** · ${o.airline} · ${o.source} · ${o.durationMin} min · stops: ${o.stops} · promos: ${o.promoTags.join(", ") || "-"}`
    )
  ];
